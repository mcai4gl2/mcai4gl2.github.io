...
class TestUtils(unittest.TestCase):
    ...
    def create_test_images(self):
        ...
        # Create a test photo (JPEG)
        photo_path = Path(self.temp_dir) / "test_photo.jpg"
        photo_array = np.random.randint(0, 256, (100, 100, 3), dtype=np.uint8)
        photo_img = Image.fromarray(photo_array, 'RGB')
        photo_img.save(photo_path, 'JPEG', quality=90)
        
        # Create a test screenshot (PNG with transparency)
        screenshot_path = Path(self.temp_dir) / "test_screenshot.png"
        screenshot_array = np.random.randint(0, 256, (100, 100, 4), dtype=np.uint8)
        screenshot_img = Image.fromarray(screenshot_array, 'RGBA')
        screenshot_img.save(screenshot_path, 'PNG')
        
        # Create a test graphic
        graphic_path = Path(self.temp_dir) / "test_graphic.png"
        # Create a simple graphic with high contrast
        graphic_array = np.zeros((100, 100, 3), dtype=np.uint8)
        graphic_array[:50, :] = [255, 255, 255]  # White top half
        graphic_array[50:, :] = [0, 0, 0]        # Black bottom half
        graphic_img = Image.fromarray(graphic_array, 'RGB')
        graphic_img.save(graphic_path, 'PNG')
    ...
    def test_detect_content_type(self):
        ...
        # Test photo detection (JPEG)
        photo_path = Path(self.temp_dir) / "test_photo.jpg"
        self.assertEqual(detect_content_type(photo_path), "photo")
        
        # Test screenshot detection (PNG with transparency)
        screenshot_path = Path(self.temp_dir) / "test_screenshot.png"
        self.assertEqual(detect_content_type(screenshot_path), "graphic")
        
        # Test graphic detection (PNG with high contrast)
        graphic_path = Path(self.temp_dir) / "test_graphic.png"
        result = detect_content_type(graphic_path)
        self.assertIn(result, ["screenshot", "graphic"])
